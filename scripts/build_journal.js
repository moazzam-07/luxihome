...
// Matching the pure white theme, Aviano Sans brand typography, and minimalism of Case Studies
const fs = require('fs');
...
console.log(`Loaded ${allArticles.length} articles for The Journal.`);

// Shared CSS matching Case Studies pure white theme & mobile menu fixes
function getWhiteSharedCss() {
  return `
    ...
    /* Minimalist Journal Card (Matches Case Studies) */
    .cs-card {
        background: #FFFFFF;
    ...
    .journal-table-card {
        background: #FFFFFF;
        border: 1px solid rgba(19, 33, 46, 0.1);
        border-radius: 16px;
        overflow-x: auto;
        box-shadow: 0 4px 16px rgba(19, 33, 46, 0.04);
    }
    .journal-table-scroll-note {
        display: none;
        padding: 8px 14px;
        font-size: 10.5px;
        text-transform: uppercase;
    ...
    .journal-comparison-table th {
        ...
        text-transform: uppercase;
        padding: 14px 18px;
        border-bottom: 1px solid rgba(19, 33, 46, 0.1);
        white-space: nowrap;
    }
    ...
    .journal-comparison-table td.font-medium {
        font-weight: 600;
        color: #13212E;
    }

    .journal-secret-sauce {
        background: #F8FAFC;
        border: 1px solid rgba(19, 33, 46, 0.1);
        border-left: 4px solid #13212E;
        border-radius: 16px;
        padding: 24px 28px;
        margin: 36px 0;
    }
    .journal-secret-sauce-badge {
        display: inline-block;
        font-family: 'aviano-sans', sans-serif;
        font-size: 10.5px;
        font-weight: 600;
                    ...
                    <div class="flex flex-col justify-between gap-40 lg:gap-50 h-full text-white">
                        <div>
                            <ul class="group/menu-items flex flex-col gap-20 sm:gap-25 lg:gap-20 whitespace-nowrap" data-target="menu-items">
                                ...
                                <li class="font-sans font-light text-24 sm:text-28 leading-none tracking-1 uppercase">
                                    <a class="transition-opacity duration-200 xl:group-hover/menu-items:opacity-20 hover:!opacity-100 ${isJournal ? 'text-[#C2A26A]' : ''}" data-barba-prevent="self" href="/journal/">Journal</a>
                                </li>
                                <li class="font-sans font-light text-24 sm:text-28 leading-none tracking-1 uppercase">
...
// White Theme Footer
function getWhiteFooterHtml(currentPath = '/journal/') {
  const isCaseStudies = currentPath.startsWith('/case-studies');
  const isJournal = currentPath.startsWith('/journal');
        ...
        <div class="cs-footer-inner">
            <div class="cs-footer-top">
                ...
                <nav class="cs-footer-nav">
                    ...
                    <a href="/case-studies/" class="cs-footer-link" ${isCaseStudies ? 'style="color: #C2A26A;"' : ''}>Case Studies</a>
                    <a href="/journal/" class="cs-footer-link" ${isJournal ? 'style="color: #C2A26A;"' : ''}>Journal</a>
...
function generateArticleHtml(art) {
  ...
  const headerHtml = getWhiteHeaderHtml(`/journal/${art.slug}/`);
  const footerHtml = getWhiteFooterHtml(`/journal/${art.slug}/`);

  const takeawaysHtml = art.takeaways ? `
    <div class="journal-takeaways">
        <div class="journal-takeaways-title">
            <span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:#C2A26A;"></span>
            <span>Key Takeaways</span>
        </div>
        <ul class="journal-takeaways-list">
            ${art.takeaways.map(t => `<li><span class="bullet">•</span><span>${t}</span></li>`).join('')}
        </ul>
    </div>
  ` : '';
  ...
  const tableHtml = art.table ? `
    <div class="journal-table-section">
        <h3 class="journal-table-title">${art.table.title}</h3>
        <div class="journal-table-card">
            <p class="journal-table-scroll-note">Swipe horizontally to compare &rarr;</p>
            <table class="journal-comparison-table">
                <thead>
                    <tr>
                        ${art.table.headers.map(h => `<th>${h}</th>`).join('')}
                    </tr>
                </thead>
                <tbody>
                    ${art.table.rows.map(row => `
                        <tr>
                            ${row.map((cell, idx) => `<td class="${idx === 0 ? 'font-medium' : ''}">${cell}</td>`).join('')}
                        </tr>
                    `).join('')}
                </tbody>
            </table>
  ...
  const secretSauceHtml = art.secretSauce ? `
    <div class="journal-secret-sauce">
        <span class="journal-secret-sauce-badge">Insider Trade Tip</span>
        <h4 class="journal-secret-sauce-title">${art.secretSauce.heading}</h4>
        <p class="journal-secret-sauce-desc">${art.secretSauce.content}</p>
    </div>
  ` : '';
  ...
  const relatedHtml = (art.relatedSlugs || []).map(slug => {
    ...
    return `
      <article class="cs-card">
          <a href="/journal/${rel.slug}/" class="cs-card-media" aria-label="View ${rel.title}">
              <img src="${rel.image}" alt="${rel.title}" class="cs-card-img" loading="lazy" />
          </a>
          <div class="cs-card-body">
              <span class="cs-card-kicker">${rel.cluster.toUpperCase()} • KOLKATA</span>
              <h2 class="cs-card-title">
                  <a href="/journal/${rel.slug}/">${rel.title}</a>
              </h2>
              <div>
                  <a href="/journal/${rel.slug}/" class="cs-card-link">
...
<head>
    ...
    <link rel="shortcut icon" href="/favicon.ico?v=luxi4" type="image/x-icon">
    <link rel="icon" type="image/svg+xml" href="/favicon.svg?v=luxi4">
    <link rel="apple-touch-icon" sizes="180x180" href="/assets/img/favicon/apple-touch-icon.png?v=luxi4">
    <link rel="icon" type="image/png" sizes="32x32" href="/assets/img/favicon/favicon-32x32.png?v=luxi4">
    <link rel="icon" type="image/png" sizes="16x16" href="/assets/img/favicon/favicon-16x16.png?v=luxi4">
    ...
    <style id="luxi-journal-detail-css">
${sharedCss}
    </style>
    ...
    <script type="application/ld+json">
    {
      ...
      "description": "${art.metaDescription.replace(/"/g, '\\"')}",
      "image": "${art.image}",
      "publisher": {
    ...
    <main id="main" class="journal-article-container">
        ...
        <header class="cs-detail-hero">
            ...
            <div class="cs-detail-badge">
                <span>${art.cluster.toUpperCase()} • KOLKATA</span>
            </div>
            <h1 class="cs-detail-title">${art.title}</h1>
            <p class="cs-detail-copy">${art.lead}</p>
        </header>
        ...
        <!-- Bottom Consultation Box (Option 2 Parity) -->
        <section class="cs-cta-box" style="margin-top: 40px; margin-bottom: 50px;">
            <div>
                <h3 class="cs-cta-title">Consult with LUXiHOME (Free Consultation)</h3>
                <p class="cs-cta-desc">
                    Whether you are planning a complete interior fit-out or building a new residence in Kolkata, schedule a 100% free consultation with LUXiHOME to review your floor plans and receive transparent cost estimates.
                </p>
            </div>
            <div>
        ...
        <section style="margin-top: 50px; margin-bottom: 60px;">
            <h3 style="font-family: 'aviano-sans', sans-serif; font-size: 14px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #8C6D38; margin-bottom: 22px;">
                Related Articles
            </h3>
...
// Generate the Updated journal/index.html Hub (Pure White, Minimalist Hero, Card Layout Matching Case Studies)
function generateHubHtml() {
  const sharedCss = getWhiteSharedCss();
  const headerHtml = getWhiteHeaderHtml('/journal/');
  const footerHtml = getWhiteFooterHtml('/journal/');

  const cardsHtml = allArticles.map(art => `
    <article class="cs-card">
        <a href="/journal/${art.slug}/" class="cs-card-media" aria-label="View ${art.title}">
            <img src="${art.image}" alt="${art.title}" class="cs-card-img" loading="lazy" />
        </a>
        <div class="cs-card-body">
            <span class="cs-card-kicker">${art.cluster.toUpperCase()} • KOLKATA</span>
            <h2 class="cs-card-title">
                <a href="/journal/${art.slug}/">${art.title}</a>
            </h2>
            <div>
                <a href="/journal/${art.slug}/" class="cs-card-link">
                    <span>Read Article</span>
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M1.16669 7H12.8334M12.8334 7L7.00002 1.16666M12.8334 7L7.00002 12.8333" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
...
${sharedCss}
    ...
    <script type="application/ld+json">
    {
      "@context": "https://schema.org",
      "@type": "CollectionPage",
      "name": "LUXiHOME Kolkata Design & Architecture Journal",
      "description": "Architectural monographs and luxury interior design guides across Kolkata.",
      "publisher": {
        "@type": "HomeAndConstructionBusiness",
        "name": "LUXiHOME Pvt. Ltd."
      }
...
<div id="page-wrap" data-barba="container" data-barba-namespace="journal">
    <main id="main" class="cs-page-container">
        ...
        <!-- Minimalist Hero: "THE JOURNAL" Written and Nothing Else -->
        <header class="cs-hero-minimal">
            <h1 class="cs-minimal-title">THE JOURNAL</h1>
        </header>

        <!-- Minimalist Journal Cards Grid (Matching Case Studies) -->
...
console.log(`Successfully built The Journal Hub at ${hubPath}`);
